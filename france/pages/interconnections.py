"""Tab 9: Interconnection Flows — 6 borders for France.

France borders: Germany, Spain, Italy, UK, Switzerland, Belgium.
France is a central European interconnection hub.
"""
...
BORDER_COLORS = {
    "Germany": COLORS["accent_blue"],
    "Spain": COLORS["accent_amber"],
    "Italy": COLORS["accent_green"],
    "UK": COLORS["accent_red"],
    "Switzerland": COLORS["accent_purple"],
    "Belgium": COLORS["accent_cyan"],
}
...
def layout():
    return html.Div(
        [
            html.Div(id="fr-interconn-kpis"),
            html.Div(
                dcc.Graph(
                    id="fr-interconn-timeseries",
                    config={
...
def register_callbacks(app):
    @app.callback(
        [
            Output(
                "fr-interconn-kpis", "children"
            ),
            Output(
                "fr-interconn-timeseries",
                "figure",
            ),
            Output(
                "fr-interconn-bar", "figure"
            ),
            Output(
                "fr-interconn-price-corr",
                "figure",
            ),
        ],
        [
    ...
    def update_interconnections(
        ...
        # Build KPIs for major borders
        border_names = [
            "Germany",
            "Spain",
            "Italy",
        ]
        border_strs = {b: "N/A" for b in border_names}
        total_str = "N/A"
        ...
        if not flows.empty:
            ts_fig = go.Figure()
            unique_countries = (
                flows["country"].unique()
            )
            for border in unique_countries:
                border_data = flows[
                    flows["country"] == border
                ]
                if not border_data.empty:
