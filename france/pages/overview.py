"""Tab 1: Market Overview — current snapshot of the French power market."""
...
def layout():
    return html.Div(
        [
            html.Div(id="fr-overview-kpis"),
            html.Div(
                [
                    html.Div(
                        dcc.Graph(
                            id="fr-overview-price-chart",
                            config={
...
def register_callbacks(app):
    @app.callback(
        [
            Output("fr-overview-kpis", "children"),
            Output(
                "fr-overview-price-chart", "figure"
            ),
            Output(
                "fr-overview-gen-chart", "figure"
            ),
            Output(
                "fr-overview-residual-chart",
                "figure",
            ),
            Output(
                "fr-overview-trend-chart", "figure"
            ),
        ],
        [
    ...
    def update_overview(start_date, end_date):
        ...
        if not price_df.empty:
            current_price = (
                f"{price_df['price_eur_mwh'].iloc[-1]:.1f}"
            )
            price_min = (
                f"{price_df['price_eur_mwh'].min():.1f}"
            )
            price_max = (
                f"{price_df['price_eur_mwh'].max():.1f}"
            )
        ...
        if not load.empty and not gen.empty:
            ...
            if ws_cols:
                ...
                merged["residual"] = merged[
                    "load_mw"
                ] - merged[ws_cols].sum(axis=1)
                residual_val = (
                    f"{merged['residual'].iloc[-1]:,.0f}"
                    " MW"
                )
        ...
        today_prices = price_df[
            price_df["timestamp"].dt.date == now
        ]
        if today_prices.empty:
            today_prices = price_df.tail(24)
        price_fig = price_bar_chart(
            today_prices, "Today's Day-Ahead Prices"
        )

        # Generation stacked area (last 24h)
        gen_24h = gen.tail(96)
