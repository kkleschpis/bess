"""Tab 6: Residual Load & System — the #1 fundamental metric for BESS."""
...
def _compute_residual(load_df, gen_df):
    ...
    ws_cols = [
        c
        for c in ["solar", "wind_onshore", "wind_offshore"]
...
def register_callbacks(app):
    ...
    def update_residual(start_date, end_date):
        ...
        gen_df = fetch_generation_by_source(start, end)
        prices_df = fetch_day_ahead_prices(start, end)
        flows_df = fetch_cross_border_flows(start, end)
        ...
        if not merged.empty:
            ...
            curve_fig = _empty_figure("Residual Load")
        ...
        if not flows_df.empty:
            country_avg = (
                flows_df.groupby("country")["flow_mw"]
                .mean()
                .sort_values()
                .reset_index()
            )
